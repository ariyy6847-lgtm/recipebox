// Extract handling for RecipeBox.
const { EventEmitter } = require('node:events');

class ExtractHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'extract' + ' payload');
 return { 'extract': String(payload).slice(0, 200) };
 }
...
module.exports = { ExtractHandler, run: (payload, opts) => new ExtractHandler().run(payload, opts) };